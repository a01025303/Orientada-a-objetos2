...
Game::Game(string elNombre, int &elID, char* elTipo): nombre(elNombre), id(elID), tipo(*elTipo), usando(""), disponible(true), siguienteJuego(NULL){}
...
OneLevelGame::OneLevelGame(){}; 
OneLevelGame::OneLevelGame(string elNombre, int &elID, char* elTipo): Game(elNombre, elID, elTipo){}


MultilevelGame::MultilevelGame(string elNombre, int &elID, char* elTipo): Game(elNombre, elID, elTipo){}
string OneLevelGame::play()
{
    return "Jugando con un solo nivel";
}
string MultilevelGame::play()
{
    return "Jugando multinivel"; 
}

GameManager::GameManager():GameList(new LinkedList){}
void GameManager::printGames()
{
    ...
    { 
        OneLevelGame* unNiv = dynamic_cast<OneLevelGame*>(GameList->Get(i));
        MultilevelGame* multiNiv = dynamic_cast<MultilevelGame*>(GameList->Get(i));
        //Si no es de un nivel
        if (unNiv != 0)
        {
            //Imprimir nombre
            cout << unNiv->getNombre() << endl; 
        }
        //Si no es multinivel
        if (multiNiv != 0)
        {
            //Imprimir nombre
            cout << multiNiv->getNombre() << endl; 
        }
    }
}